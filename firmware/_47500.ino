...
byte chip[chipCount] = { 0x20, 0x21, 0x22 };
byte bank[2] = { 0x12, 0x13 };

int alarmPin[rowCount][colCount] = {
  { 1, 2, 3, 4, 5, 6, 7, 8 }, // set row 1
  { 1, 2, 3, 4, 5, 6, 7, 8 }, // set row 2
  { 1, 2, 3, 4, 5, 6, 7, 8 }  // set row 3
};
...
int flashReg[3][8] = {
  { 0, 0, 0, 0, 0, 0, 0, 0 }, // initialise group 1
  { 0, 0, 0, 0, 0, 0, 0, 0 }, // initialise group 2
  { 0, 0, 0, 0, 0, 0, 0, 0 }  // initialise group 3
};
...
void setup() {
  ...
  for (int row = 0; row < rowCount; row++) {
    for (int col = 0; col < colCount; col++) {
        pinMode(lampPin[row][col], OUTPUT);
    }
  }
  // loop over the pin array and set them all to input:
  for (int row = 0; row < rowCount; row++) {
    for (int col = 0; col < colCount; col++) {
        pinMode(alarmPin[row][col], INPUT);
    }
  }
  // set up local pins
...
void alarms() {
  ...
  for (int row = 0; row < rowCount; row++) {
    for (int col = 0; col < colCount; col++) {
        if (!digitalRead(alarmPin[row][col])) {   // IF ALARM PIN NOT HIGH
        flashReg[row][col] = true;
  ...
  for (int row = 0; row < rowCount; row++) {
    for (int col = 0; col < colCount; col++) {
        if ((flashReg[row][col]) && (digitalRead(acceptPin))) {
        steadyReg[row][col] = true;
  ...
  for (int row = 0; row < rowCount; row++) {
    for (int col = 0; col < colCount; col++) {
        if ((steadyReg[row][col]) && (digitalRead(resetPin)) && alarmPin[row][col]) {
        steadyReg[row][col] = false;
        flashReg[row][col] = false;
      }
    }
  }
  ...
  for (int row = 0; row < rowCount; row++) {
    for (int col = 0; col < colCount; col++) {
      digitalWrite(lampPin[row][col], (flashReg[row][col] && FLASHER) || (steadyReg[row][col]));
    }
  }
}
